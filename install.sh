...
if [ ! "$(command -v rustc)" ]
    ...
    # 0 = 是   1 = 否
	      ...
	      then
		    ...
		    tmux send-keys -t gpu01 'export CUDA_VISIBLE_DEVICES=0 && /root/miner/aleo-pool-prover_ubuntu1804_gpu-20221123 --pool 128.14.72.188:30009 --account_name ${account_name} --miner_name gpu01 > aleo0.log 2>&1' C-m
		    sleep 5
		    tmux send-keys -t gpu02 'export CUDA_VISIBLE_DEVICES=1 && /root/miner/aleo-pool-prover_ubuntu1804_gpu-20221123 --pool 128.14.72.188:30009 --account_name ${account_name} --miner_name gpu02 > aleo1.log 2>&1' C-m
		    sleep 5
		    tmux send-keys -t gpu03 'export CUDA_VISIBLE_DEVICES=2 && /root/miner/aleo-pool-prover_ubuntu1804_gpu-20221123 --pool 128.14.72.188:30009 --account_name ${account_name} --miner_name gpu03 > aleo2.log 2>&1' C-m
		    sleep 5
